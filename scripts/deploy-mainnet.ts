 ...
 *   A) Pre-populate `registrations` big_map at origination by querying TED GraphQL.
...
const SET_CHILD_RECORD_PROXY = "KT1QHLk1EMUA8BPH3FvRUeUmbTspmAhb7kpd";
...
const CONTRACT_JSON = resolve(
    __dirname,
    "../contract/output/Commit/step_001_cont_0_contract.json",
);
...
async function fetchAllHackTezOwners(): Promise<Map<string, number>> {
    console.log("  Querying TED GraphQL for all current *.hack.tez owners...");
    ...
    while (true) {
        const res = await fetch(TED_GRAPHQL, {
            ...
            body: JSON.stringify({
                query: `query HackTezOwners($suffix: String!, $skip: Int!, $take: Int!) {
                    domains(
                ...
                variables: { suffix: PARENT_SUFFIX, skip, take: PAGE_SIZE },
            }),
        });
        const json = (await res.json()) as {
            data?: { domains: { items: DomainItem[] } };
        ...
        console.log(`    Page skip=${skip}: fetched ${items.length} records`);
        if (items.length < PAGE_SIZE) break;
        skip += PAGE_SIZE;
    }

    console.log(`  Found ${ownerCounts.size} unique owner(s) across ${[...ownerCounts.values()].reduce((a, b) => a + b, 0)} domain(s).`);
    return ownerCounts;
}
...
async function main() {
    ...
    // ─── Step A: Pre-populate registrations from TED ────────────────────
    console.log(`\n🔍 Step A: Fetching existing hack.tez owners from TED...`);
    const preDeployOwners = await fetchAllHackTezOwners();
    ...
    const metadataJson = JSON.stringify({
        ...
        version: "2.0.0",
        description: "Free sub-domain registrar for hack.tez on mainnet. Owner=sender model.",
        interfaces: ["TZIP-016"],
    });
    ...
    const postDeployOwners = await fetchAllHackTezOwners();
    ...
    if (newOwners.length === 0) {
        ...
        for (const { address: wallet, count } of newOwners) {
            ...
            const updateOp = await contract.methods
                .set_registration_count(wallet, count)
                .send();
    ...
    console.log(`     → update_operators → add_operator(owner=<admin>, operator=${contractAddress}, token_id=<hack.tez token_id>)`);
