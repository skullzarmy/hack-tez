...
function drawPattern(
    ...
    const traceCount = Math.round((isMonoPreset ? 8 : 14) + density * (isMonoPreset ? 16 : 22));
    const nodeCount = Math.round((isMonoPreset ? 10 : 16) + density * (isMonoPreset ? 14 : 26));
    ...
    for (let index = 0; index < traceCount; index += 1) {
        ...
        const segments = 3 + Math.round(seededUnit(seed, index + 91) * (2 + density * 2));
        ...
        for (let step = 0; step < segments; step += 1) {
            ...
            const direction = seededUnit(seed, index * 19 + step * 29 + 5) > 0.5 ? 1 : -1;
            const distance = Math.round(minStep + seededUnit(seed, index * 7 + step * 23 + 9) * (maxStep - minStep));
            const nx = horizontal ? Math.max(0, Math.min(width, x + distance * direction)) : x;
            const ny = horizontal ? y : Math.max(0, Math.min(height, y + distance * direction));
            x = nx;
            y = ny;
            ctx.lineTo(x, y);
        }
    ...
    const spacing = Math.max(3, Math.round((isGlitchPreset ? 7 : 9) - glitch * 3));
    ...
    if (glitch > 0.02) {
        ...
        if (snapshotCtx) {
            ...
            for (let index = 0; index < bandCount; index += 1) {
                ...
                ctx.drawImage(snapshot, 0, y, width, bandHeight, shift, y, width, bandHeight);
                ctx.restore();
            }
            ...
            for (let index = 0; index < splitBands; index += 1) {
                ...
                ctx.drawImage(snapshot, 0, y, width, bandHeight, splitPx, y, width, bandHeight);
    ...
    const pulseCount = Math.max(1, Math.round(1 + glitch * (isGlitchPreset ? 10 : 6)));
...
function drawCard(
    ...
    const accentOverride = state.preset === "mono-poster" ? basePreset.accent : state.templateColor;
    ...
    if (avatarImage) {
        ...
        ctx.fillText(fullName.slice(0, 2).toUpperCase(), avatarX + avatarSize / 2, avatarY + avatarSize / 2);
    }
    ctx.restore();
    ...
    const titleLines = wrapCanvasText(ctx, state.title, titleMaxWidth, state.format === "og" ? 2 : 3);
    titleLines.forEach((line, index) => {
        ctx.fillText(line, contentX, titleTop + index * Math.round(width * 0.076));
    });

    const subtitleTop = titleTop + titleLines.length * Math.round(width * 0.076) + 20;
    ...
    const subtitleLines = wrapCanvasText(ctx, state.subtitle, contentRight - contentX, state.format === "og" ? 3 : 4);
    subtitleLines.forEach((line, index) => {
        ctx.fillText(line, contentX, subtitleTop + index * Math.round(width * 0.039));
    });
    ...
    ctx.fillText(`// ${fullName}`, contentX, panelY + panelHeight - panelInset - 14);
...
export function ProfileShareStudio({ label, fullName, displayName, avatarUrl, bio, status }: ProfileShareStudioProps) {
    ...
    useEffect(() => {
        ...
        drawCard(canvasRef.current, state, fullName, profileUrl, statusLabel, avatarImage);
        ...
        >
            <div style={{ display: "flex", justifyContent: "space-between", gap: "1rem", alignItems: "center" }}>
                <div>
                    ...
                    <p style={{ margin: 0, color: "var(--fg-2)", fontSize: "0.8rem", lineHeight: 1.5 }}>
            ...
            {open && (
                <div style={{ marginTop: "0.9rem", display: "flex", flexWrap: "wrap", gap: "1rem", alignItems: "flex-start" }}>
                    <div style={{ flex: "1 1 320px", minWidth: "280px", display: "flex", flexDirection: "column", gap: "0.8rem" }}>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: "0.9rem" }}>
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(event) =>
                                        setState((current) => {
                                            const nextPreset = event.target.value as ProfileSharePreset;
                                            const nextColor =
                            ...
                            {state.preset === "mono-poster" ? (
                                <label style={{ display: "flex", alignItems: "center", gap: "0.5rem", color: "var(--fg-2)", fontSize: "0.75rem" }}>
                                    <input
                                        ...
                                        onChange={(event) =>
                                            setState((current) => ({ ...current, monoInvert: event.target.checked }))
                                        }
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(event) =>
                                            setState((current) => ({ ...current, templateColor: event.target.value }))
                                        }
                                        style={{
                                            width: "100%",
                                            height: "2rem",
                        ...
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: "0.75rem" }}>
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(event) =>
                                        setState((current) => ({ ...current, circuitGlow: Number(event.target.value) }))
                                    }
                                    style={RANGE_STYLE}
                                />
                            </div>
                        ...
                        <div style={{ color: "var(--fg-3)", fontSize: "0.68rem", lineHeight: 1.4 }}>
                        ...
                        <div>
                            ...
                            <input
                                ...
                                maxLength={64}
                                value={state.cta}
                                onChange={(event) => setState((current) => ({ ...current, cta: event.target.value }))}
                                style={INPUT_STYLE}
                            />
                        </div>

                        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
                            <button type="button" onClick={handleDownload} style={ACTION_BUTTON}>
                                <Download size={14} /> Download
                            </button>
                            <button type="button" onClick={handleCopyImage} style={ACTION_BUTTON}>
                                <Copy size={14} /> Copy Image
                            </button>
                            <button type="button" onClick={handleShareX} style={ACTION_BUTTON}>
                                <Share2 size={14} /> Share to X
                            </button>
                        </div>

                        <div style={{ color: "var(--fg-3)", fontSize: "0.72rem", lineHeight: 1.45 }}>
                            Posts link to {profileUrl}. The exported image is local-only and is not stored server-side.
                        </div>
